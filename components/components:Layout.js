...
// Importa i dati per il menu
import { services } from '../lib/data';

export default function Layout({ children, hasBackground = false, backgroundImage = null }) {
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <nav className="hidden md:flex space-x-8">
              <Link href="/" className={`text-white hover:text-[#d2ad40] transition-colors ${currentPage === '/' ? 'text-[#d2ad40] font-semibold' : ''}`}>HOME</Link>
              <Link href="/about" className={`text-white hover:text-[#d2ad40] transition-colors ${currentPage === '/about' ? 'text-[#d2ad40] font-semibold' : ''}`}>CHI SIAMO</Link>
              <div className="relative">
                <button 
                  onClick={() => setIsServicesDropdownOpen(!isServicesDropdownOpen)}
                  className={`flex items-center text-white hover:text-[#d2ad40] transition-colors ${currentPage.startsWith('/services') ? 'text-[#d2ad40] font-semibold' : ''}`}
                >
                  SERVIZI
              ...
              <Link href="/portfolio" className={`text-white hover:text-[#d2ad40] transition-colors ${currentPage === '/portfolio' ? 'text-[#d2ad40] font-semibold' : ''}`}>LAVORI</Link>
              <Link href="/blog" className={`text-white hover:text-[#d2ad40] transition-colors ${currentPage === '/blog' ? 'text-[#d2ad40] font-semibold' : ''}`}>BLOG</Link>
            </nav>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="px-4 py-4 space-y-4">
              ...
              <div>
                <button onClick={() => setIsServicesDropdownOpen(!isServicesDropdownOpen)} className="flex items-center text-white w-full text-left">
      ...
      {/* Main Content */}
      <main className={hasBackground ? "pt-20" : "pt-24 pb-20"}>
        {children}
      </main>
